    ...
    def send_email(self, options):
        ...
        if email and email.strip():
            ...
            partner.with_context(mail_post_autofollow=True).message_post(
                ...
                body=body_html,
                subject=  '%s rappels de paiement - %s' % (self.env.company.name, partner.name),
            ...
            return True
        raise UserError(_('Could not send mail to partner %s - %s because it does not have any email address defined')% (partner.display_name,partner.id))
